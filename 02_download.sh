...
#SBATCH -p pi_tami
#SBATCH --mem=300GB
#SBATCH --output=/home/apolonio/orcd/scratch/20_440/project_logs/diamond_database.log

set -euo pipefail

export PATH=$PATH:/home/apolonio/sratoolkit.3.2.1-alma_linux64/bin #location of SRA toolkit
...
# Metadata.csv columns (tab-separated, with header):
#   BioProject  SRR  Type  Name  Title  Environment  Latitude_N  Longitude_W  Dataset

echo ""
echo " Starting downloads "

tial -n le IFS=$'\t' read -r bioproject srr t+2 "$METADATA" | whiype name title env lat lon dataset; do
